...
async fn main() -> SdkResult<()> {
    ...
    // `--scripts-dir` (Forge) > config > CHAOSNEXUS_SCRIPTS_DIR (Suite) > sibling-checkout defaults.
    ...
    // Create scripts layout before Landlock so PathFd targets exist, then jail FS.
    let scripts_path = std::path::Path::new(scripts_dir);
    let _ = chaosnexus_anvil::scripting::sandbox::ensure_scripts_layout(scripts_path);
    let _sandbox_status =
        chaosnexus_anvil::scripting::sandbox::apply_filesystem_sandbox_if_available(scripts_path);
    ...
    // events dir already ensured by ensure_scripts_layout / Landlock prepare
