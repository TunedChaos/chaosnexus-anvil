...
pub fn build_config_map_from(
    ...
    // Configured secrets in chaosnexus-anvil.toml: values are ENV VAR NAMES.
    // Never inject TOML literals as secret material.
    for (key, env_name) in plugin_secrets {
        ...
        {
            eprintln!(
                "[chaosnexus-anvil] Ignoring invalid secrets mapping for '{key}':                  value must be an environment variable name, not secret material"
            );
            continue;
        }
