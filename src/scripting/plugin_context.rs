...
pub fn namespaced_key(key: &str, caps: &CapabilitySet) -> String {
    // `shared::` prefix is only honored when SharedGlobal is granted (no bypass).
    if caps.has(Capability::SharedGlobal) {
        return key.to_string();
    }
    if key.starts_with("shared::") {
        // Strip attempt to reach shared namespace without the capability:
        // fall through to plugin-prefixed key using the raw name.
    }
...
mod tests {
    ...
    #[test]
    fn shared_prefix_requires_capability() {
        with_plugin_context("a", || {
            let caps = CapabilitySet::default();
            assert_eq!(namespaced_key("shared::x", &caps), "a::shared::x");
            let mut shared = CapabilitySet::default();
            shared.granted.insert(Capability::SharedGlobal);
            assert_eq!(namespaced_key("shared::x", &shared), "shared::x");
        });
    }
}
