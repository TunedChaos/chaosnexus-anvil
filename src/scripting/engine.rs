...
fn configure_module_resolvers(engine: &mut Engine) {
    ...
    // Plugin-local imports: rooted at scripts/plugins (no absolute / CWD escape).
    // Relative imports resolve under plugins_root; `..` escaping is rejected by path join checks below via FileModuleResolver base.
    resolvers.push(FileModuleResolver::new_with_path(paths::plugins_root()));
