...
/// Allowlisted process binaries for mcp_connect (permissions.process, else permissions.shell).
fn process_binary_allowed(ctx: &NativeContext, plugin_name: &str, command: &str) -> bool {
    let base = std::path::Path::new(command)
        .file_name()
        .and_then(|s| s.to_str())
        .unwrap_or(command);
    let perms = ctx.plugins.read().unwrap();
    let Some(config) = perms.get(plugin_name) else {
        return false;
    };
...
fn do_connect(
    ...
    if !process_binary_allowed(ctx, plugin_name, command) {
        return Err(format!(
            "Security Violation: process binary '{}' is not in permissions.process              (or permissions.shell) allowlist for plugin '{}'",
            command, plugin_name
        )
        .into());
    }
